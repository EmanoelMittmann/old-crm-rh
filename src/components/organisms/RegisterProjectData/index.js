...
const RegisterProjectData = ({ data, typeOptions, statusOptions }) => {
    const { values, handleChange, errors, touched, setFieldTouched, setFieldValue, setFieldError } = data

    useEffect(() => {
        if (errors.date_end_performed === 'after or equal to date validation failed') return setFieldError('date_end_performed', 'Os campos "Data início efetivo" e "Data final efetivo" devem ser preenchidos!')
    }, [errors.date_end_performed])
